...
for m in $MAP_FILES; do
	...
	if [[ ! -f ${mapping_dir_out}/${FILE}.sam ]]; then
		...
		if [[ "${strand_num}" = "1" ]]; then
			...
			${BOWTIE} -x ${species_location}/bowtie2/${species} --threads ${THREADS} -1 ${mapfiles}/${m}*1.fastq.gz -2 ${mapfiles}/${m}*2.fastq.gz --time -S ${mapping_dir_out}/${FILE}.sam 2> ${mapping_logs}/${FILE}-Results.log
		fi
