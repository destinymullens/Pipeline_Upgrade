	...
	if [[ ! -f ${htseq_dir_out}/counts/${FILE}-htseq.csv ]]; then
		...
		printf "%s\n" "Counting of ${FILE} beginning..."
			if [[ "${strand_num}" = "1" ]]; then	
				${HTSEQ_LOC} ${i} ${species_location}/genes.gtf --stranded=no -m intersection-strict -f sam -i gene_id --additional-attr=GeneID -o ${htseq_dir_out}/sam_files/${FILE}-htseq.sam -c ${htseq_dir_out}/counts/${FILE}-htseq.csv --with-header
			else
				${HTSEQ_LOC} --stranded=yes -m intersection-strict -f sam -i gene_id --additional-attr=GeneID ${i} ${species_location}/genes.gtf -o ${htseq_dir_out}/sam_files/${FILE}-htseq.sam -c ${htseq_dir_out}/counts/${FILE}-htseq.csv --with-header
			fi
...
#### This section is saving the htseq counts file & individual metrics for overall summary output
    ...
    printf "%s\t" $(cat ${htseq_dir_out}/${FILE}-htseq.MT.count) >> ${summary_file} ## Print Total mito reads to summary
